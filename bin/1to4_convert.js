...
parse(f1to4, {comment:"#"}, function(csv_err, csv_data){
  ...
  for(var cd=1; cd< csv_data.length ; cd+=2){
    ...
    if(dataIn[0]===dataOut[0] && dataIn[1]===dataOut[1] && dataIn[2]===dataOut[2]){
      var lStationName = dataIn[2].substr(0,dataIn[2].indexOf("("));
      ...
      for(var h=0; h < 24; h++){
        ...
        /*
        fs.appendFile("data/"+fileName, logdata, function (err) {
          if (err){
            console.log(err);
          }
        });
...
/*
fs.readFile('source/station_info.json', 'utf8', function (err,data) {
  if (err) {
...
//  console.log("%j", s_meta["망원"]);
  ...
  var sourceFile='source/2014_1TO4.csv';
  fs.readFile(sourceFile, 'utf8', function (terr,tdata) {
    if (terr) {
      return console.log(terr);
    }

    parse(tdata, {comment:"#"}, function(csv_err, csv_data){
      if (csv_err) {
        return console.log(csv_err);
      }
      //console.log(csv_data.length);
      ...
      for(var cd=1; cd< 100 ; cd+=2){
        ...
        if(dataIn[0]===dataOut[0] && dataIn[1]===dataOut[1] && dataIn[2]===dataOut[2]){
          ...
          var ldateTemp = dataIn[0].split('-');
          for(var h=0; h < 24; h++){
            var ldate = new Date(ldateTemp[0],ldateTemp[1],ldateTemp[2],h,0,0);
            //console.log("%j"+ldate);
            // - 로 되어 있는 값들 0으로 변경.
            var people_in = dataIn[4+h];
            people_in = people_in.trim();
            ...
            //console.log("%j",s_logs);
            var fileName = "1to4_"+ldate.getFullYear()+ldate.getMonth()+ldate.getDate()+".log";
            var logdata = JSON.stringify(s_logs)+"\n";
